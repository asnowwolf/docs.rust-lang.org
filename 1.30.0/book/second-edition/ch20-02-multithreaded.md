## [Turning Our Single-Threaded Server into a Multithreaded Server](ch20-02-multithreaded.html#turning-our-single-threaded-server-into-a-multithreaded-server)
...
### [Simulating a Slow Request in the Current Server Implementation](ch20-02-multithreaded.html#simulating-a-slow-request-in-the-current-server-implementation)
...
### [Improving Throughput with a Thread Pool](ch20-02-multithreaded.html#improving-throughput-with-a-thread-pool)
...
#### [Code Structure If We Could Spawn a Thread for Each Request](ch20-02-multithreaded.html#code-structure-if-we-could-spawn-a-thread-for-each-request)
...
#### [Creating a Similar Interface for a Finite Number of Threads](ch20-02-multithreaded.html#creating-a-similar-interface-for-a-finite-number-of-threads)
...
#### [Building the `ThreadPool` Struct Using Compiler Driven Development](ch20-02-multithreaded.html#building-the-threadpool-struct-using-compiler-driven-development)
...
#### [Validating the Number of Threads in `new`](ch20-02-multithreaded.html#validating-the-number-of-threads-in-new)
...
#### [Creating Space to Store the Threads](ch20-02-multithreaded.html#creating-space-to-store-the-threads)
...
#### [A `Worker` Struct Responsible for Sending Code from the `ThreadPool` to a Thread](ch20-02-multithreaded.html#a-worker-struct-responsible-for-sending-code-from-the-threadpool-to-a-thread)
...
#### [Sending Requests to Threads via Channels](ch20-02-multithreaded.html#sending-requests-to-threads-via-channels)
...
#### [Implementing the `execute` Method](ch20-02-multithreaded.html#implementing-the-execute-method)
...
As you saw in Listing 17-15, we can write methods that use the syntax `self: Box<Self>`, which allows the method to take
ownership of a `Self` value stored
