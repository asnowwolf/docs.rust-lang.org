## [Shared-State Concurrency](ch16-03-shared-state.html#shared-state-concurrency)
...
### [Using Mutexes to Allow Access to Data from One Thread at a Time](ch16-03-shared-state.html#using-mutexes-to-allow-access-to-data-from-one-thread-at-a-time)
...
#### [The API of `Mutex<T>`](ch16-03-shared-state.html#the-api-of-mutext)
...
#### [Sharing a `Mutex<T>` Between Multiple Threads](ch16-03-shared-state.html#sharing-a-mutext-between-multiple-threads)
...
#### [Multiple Ownership with Multiple Threads](ch16-03-shared-state.html#multiple-ownership-with-multiple-threads)
...
on: the first inline error says ``std::rc::Rc<std::sync::Mutex<i32>>` cannot be sent between threads safely`. The reason
for this is in the next important
part to focus on, the error message. The distilled error message says `the trait bound `Send` is not satisfied`. We’ll
talk about `Send` in the next
...
#### [Atomic Reference Counting with `Arc<T>`](ch16-03-shared-state.html#atomic-reference-counting-with-arct)
...
### [Similarities Between `RefCell<T>`/`Rc<T>` and `Mutex<T>`/`Arc<T>`](ch16-03-shared-state.html#similarities-between-refcelltrct-and-mutextarct)
