## [Running Code on Cleanup with the `Drop` Trait](ch15-03-drop.html#running-code-on-cleanup-with-the-drop-trait)
...
### [Dropping a Value Early with `std::mem::drop`](ch15-03-drop.html#dropping-a-value-early-with-stdmemdrop)
...
between the `CustomSmartPointer created.` and `CustomSmartPointer dropped before the end of main.` text, showing that
the `drop` method code is called to
