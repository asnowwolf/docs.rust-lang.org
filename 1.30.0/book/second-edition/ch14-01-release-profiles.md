## [Customizing Builds with Release Profiles](ch14-01-release-profiles.html#customizing-builds-with-release-profiles)
...
Cargo has two main profiles: the `dev` profile Cargo uses when you run `cargo build` and the `release` profile Cargo
uses when you run `cargo build --release`. The `dev` profile is defined with good defaults for development,
