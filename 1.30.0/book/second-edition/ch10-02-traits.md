...
### [Defining a Trait](ch10-02-traits.html#defining-a-trait)
...
import the trait into their scope first. They would do so by specifying `use aggregator::Summary;`, which then would
enable them to implement `Summary` for
...
### [Default Implementations](ch10-02-traits.html#default-implementations)
...
### [Fixing the `largest` Function with Trait Bounds](ch10-02-traits.html#fixing-the-largest-function-with-trait-bounds)
...
values in the slice that we pass into the function implement the `PartialOrd`*and* `Copy` traits, like `i32` and `char`
do.
...
### [Using Trait Bounds to Conditionally Implement Methods](ch10-02-traits.html#using-trait-bounds-to-conditionally-implement-methods)
